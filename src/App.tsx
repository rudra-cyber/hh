...
export default function App() {
  ...
  // Current tab view: "ledger" | "analytics" | "strategies"
  const [activeTab, setActiveTab] = useState<"ledger" | "analytics" | "strategies">("ledger");
        ...
        try {
          ...
          if (cloudStrats.length > 0) {
            ...
            DEFAULT_STRATEGIES.forEach(s => {
              ...
              batch.set(docRef, { ...s, userId: user.uid });
            });
            try {
              await batch.commit();
            } catch (err) {
  ...
  const handleSaveTrade = async (formData: Omit<Trade, "id" | "createdAt"> & { id?: string }) => {
    ...
    if (currentUser && db) {
      try {
        ...
        await setDoc(docRef, completeTrade);
      } catch (err) {
  ...
  const handleAddStrategy = async (name: string, description: string) => {
    ...
    if (currentUser && db) {
      try {
        ...
        await setDoc(docRef, { ...newStrat, userId: currentUser.uid });
      } catch (err) {
  ...
  const filteredTrades = React.useMemo(() => {
    return trades.filter((t) => {
      ...
      return matchesSearch && matchesWinLoss && matchesStrategy;
    });
  }, [trades, searchQuery, statusFilter, strategyFilter]);

  return (
    <div className={`min-h-screen ${styles.bg} flex flex-col font-sans selection:bg-blue-650/30 transition-colors duration-200`}>
        ...
        <div className="flex items-center space-x-2 flex-wrap">
          ...
          <div className={`flex p-1 border rounded-xl space-x-1 items-center ${styles.pillBg}`} style={{ borderColor: activeTheme === 'light' ? '#CBD5E1' : '#1e293b' }}>
            ...
            <button
              ...
              title="Light Terminal Pro Theme"
            />
          </div>
      ...
      <main className="max-w-7xl w-full mx-auto p-6 space-y-6 flex-1">
        ...
        <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center border-b ${styles.border} pb-4 space-y-4 sm:space-y-0`}>
          ...
          <div className={`flex ${styles.headerBg} p-1 border rounded-xl`} style={{ borderColor: activeTheme === 'light' ? '#E2E8F0' : '#1e293b' }}>
            <button
              onClick={() => setActiveTab("ledger")}
              className={`py-2 px-4 rounded-lg text-xs font-bold tracking-wide transition flex items-center space-x-1.5 cursor-pointer ${
                activeTab === "ledger" ? (activeTheme === "light" ? "bg-blue-600 text-white shadow-sm" : "bg-blue-600 text-white shadow-md") : "text-slate-400 hover:text-slate-200"
              }`}
            >
            ...
            <button
              ...
              className={`py-2 px-4 rounded-lg text-xs font-bold tracking-wide transition flex items-center space-x-1.5 cursor-pointer ${
                activeTab === "analytics" ? (activeTheme === "light" ? "bg-blue-600 text-white shadow-sm" : "bg-blue-600 text-white shadow-md") : "text-slate-400 hover:text-slate-200"
              }`}
            >
            ...
            <button
              ...
              className={`py-2 px-4 rounded-lg text-xs font-bold tracking-wide transition flex items-center space-x-1.5 cursor-pointer ${
                activeTab === "strategies" ? (activeTheme === "light" ? "bg-blue-600 text-white shadow-sm" : "bg-blue-600 text-white shadow-md") : "text-slate-400 hover:text-slate-200"
              }`}
            >
          ...
          <button
            ...
            className="w-full sm:w-auto px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold tracking-wide flex items-center justify-center space-x-1.5 cursor-pointer shadow-lg shadow-blue-900/20 hover:shadow-blue-900/30 transition-all duration-300 transform hover:-translate-y-0.5"
          >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredTrades.map((t) => (
                <div 
                   key={t.id} 
                   className={`${styles.cardBg} ${styles.cardHover} border ${styles.border} rounded-2xl p-5 shadow-xl transition-all duration-300 md:flex justify-between items-start space-y-4 md:space-y-0`}
                >
                  ...
                  <div className="space-y-2 flex-1 pr-6">
                    ...
                    {/* Entry Rules Confirmation Checklist / Badges */}
                      ...
                      <div className="mt-2.5 space-y-1">
                        ...
                        <div className="flex flex-wrap gap-1 pt-0.5">
                          {t.entryRules.map((rule, idx) => (
                            <span 
                              key={idx} 
                              className="text-[9px] font-mono bg-emerald-500/10 text-[#00b074] border border-emerald-500/20 rounded px-1.5 py-0.5 uppercase tracking-wide font-extrabold"
                            >
                              ✓ {rule}
                            </span>
                          ))}
                    ...
                    {/* Multi-Timeframe Dedicated Charts and Generic Snapshots Dashboard */}
                    <div className="space-y-3 mt-3 pt-2">
                      
                      {/* Big Timeframe scenario text details */}
                      {t.bigTimeFrameScenario && (
                        <div className={`bg-amber-500/5 border ${activeTheme === 'light' ? 'border-amber-400/30' : 'border-amber-500/20'} p-3 rounded-xl`}>
                      ...
                      {t.entryReason && (
                        <div className={`bg-blue-500/5 border ${activeTheme === 'light' ? 'border-blue-400/30' : 'border-blue-500/20'} p-3 rounded-xl`}>
                      ...
                      {/* Display of dailyChart, fourHourChart, oneHourChart, fifteenMinChart, and images */}
                      {(t.dailyChart || t.fourHourChart || t.oneHourChart || (t as any).fifteenMinChart || (t.images && t.images.length > 0)) && (
                        <div className="pt-1">
                          <span className={`text-[10px] font-bold uppercase tracking-wider ${styles.textMuted} block mb-1.5`}>
                            📸 Multi-Timeframe Chart Evidence:
                          </span>
                          ...
                          <div className="flex gap-2 flex-wrap">
                            ...
                            {t.dailyChart && (
                              <div 
                                ...
                                className="relative rounded-xl overflow-hidden border border-slate-800 hover:border-blue-500 cursor-zoom-in h-16 w-28 group bg-black/30"
                              >
                                <img src={t.dailyChart} alt="Daily chart" className="h-full w-full object-cover transition duration-350 group-hover:scale-110" />
                                ...
                                <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 flex items-center justify-center transition focus:outline-none">
                            ...
                            {t.fourHourChart && (
                              <div 
                                ...
                                className="relative rounded-xl overflow-hidden border border-slate-800 hover:border-blue-500 cursor-zoom-in h-16 w-28 group bg-black/30"
                              >
                                <img src={t.fourHourChart} alt="4H chart" className="h-full w-full object-cover transition duration-350 group-hover:scale-110" />
                            ...
                            {t.oneHourChart && (
                              <div 
                                ...
                                className="relative rounded-xl overflow-hidden border border-slate-800 hover:border-blue-500 cursor-zoom-in h-16 w-28 group bg-black/30"
                              >
                                <img src={t.oneHourChart} alt="1H chart" className="h-full w-full object-cover transition duration-350 group-hover:scale-110" />
                            ...
                            {(t as any).fifteenMinChart && (
                              <div 
                                ...
                                className="relative rounded-xl overflow-hidden border border-slate-800 hover:border-blue-500 cursor-zoom-in h-16 w-28 group bg-black/30"
                              >
                                <img src={(t as any).fifteenMinChart} alt="15M chart" className="h-full w-full object-cover transition duration-350 group-hover:scale-110" />
                            ...
                            {t.images && t.images.map((img, idx) => (
                              <div 
                                ...
                                className="relative rounded-xl overflow-hidden border border-slate-800 hover:border-blue-500 cursor-zoom-in h-16 w-28 group bg-black/30"
                              >
                                <img src={img} alt="Supporting screenshot" className="h-full w-full object-cover transition duration-350 group-hover:scale-110" />
