...
export default function AIAssistant({ onExtractionComplete }: AIAssistantProps) {
  ...
  return (
    <div className="bg-[#161B22] border border-slate-800 rounded-2xl p-5 shadow-2xl space-y-4 ai-assistant-zone" onPaste={handlePaste}>
      {/* Title block */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <textarea
            ...
            className="w-full text-sm h-32 bg-[#0A0E14] border border-slate-800 focus:border-blue-500 focus:outline-none rounded-xl p-3 text-white placeholder-slate-600 resize-none font-sans"
          ></textarea>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="flex-1 flex items-center justify-center space-x-2 py-2 px-3 text-xs font-medium text-slate-300 bg-[#1C2128] hover:bg-[#242b36] rounded-xl border border-slate-800 transition cursor-pointer"
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center space-x-2 py-2 px-3 text-xs font-medium rounded-xl border transition ${
                showCamera 
                ? 'bg-[#00b074] hover:bg-[#008f5d] border-transparent text-white' 
                : 'text-slate-300 bg-[#1C2128] hover:bg-[#242b36] border-slate-800'
              }`}
            >
          ...
          {showCamera ? (
            ...
            <div className="w-full h-full relative group">
              ...
              <button
                ...
                className="absolute top-2 right-2 text-white bg-black/60 hover:bg-black/90 p-1.5 rounded-full"
              >
                <RefreshCw className="h-3 w-3" />
              </button>
            </div>
          ) : (
            <div className="p-4 text-center cursor-pointer" onClick={() => { if (fileInputRef.current) fileInputRef.current.click() }}>
              <div className="mx-auto w-10 h-10 border border-dashed border-slate-800 rounded-full flex items-center justify-center mb-2">
                <Upload className="h-4 w-4 text-slate-500 animate-bounce" />
              </div>
              <p className="text-xs text-slate-400">Captured Chart or Screenshot preview</p>
      ...
      <div className="pt-2 flex flex-col space-y-2">
        <button
          ...
          className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-[#1C2128] disabled:text-slate-600 disabled:cursor-not-allowed text-white py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center space-x-2 cursor-pointer shadow-lg shadow-blue-900/10 transition duration-200"
        >
          {isLoading ? (
            <>
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/30 p-2.5 rounded-xl flex items-start space-x-2 text-xs text-red-200">
        ...
        {successMsg && (
          <div className="bg-[#12533e]/30 border border-[#00b074]/30 p-2.5 rounded-xl flex items-center space-x-2 text-xs text-[#00b074]">
