  ...
  const downloadStrategyPDFReport = (strat: Strategy, stratTrades: Trade[]) => {
    ...
    const tradeSheetsHtml = stratTrades.map((t, idx) => {
      ...
      const flowRulesHtml = t.entryRules && t.entryRules.length > 0
        ? t.entryRules.map(r => `<span style="background-color: rgba(0, 176, 116, 0.12); color: #00b074; font-size: 9px; padding: 3px 8px; border-radius: 4px; font-weight: bold; margin-right: 4px; border: 1px solid rgba(0, 176, 116, 0.25);">✓ ${r}</span>`).join("")
        : `<span style="color: #8b949e; font-size: 10px; font-style: italic;">No specific checklist logged</span>`;

      // Build charts HTML
      let chartSegments = [];
      if (t.fourHourChart) {
        chartSegments.push(`
          <div style="text-align: center; border: 1px solid #30363d; border-radius: 8px; padding: 8px; background-color: #0d1117;">
            <div style="font-size: 9px; font-weight: bold; color: #8b949e; margin-bottom: 4px; text-transform: uppercase;">4 Hour Chart</div>
      ...
      if (t.oneHourChart) {
        chartSegments.push(`
          <div style="text-align: center; border: 1px solid #30363d; border-radius: 8px; padding: 8px; background-color: #0d1117;">
            <div style="font-size: 9px; font-weight: bold; color: #8b949e; margin-bottom: 4px; text-transform: uppercase;">1 Hour Chart</div>
      ...
      if (t.fifteenMinChart) {
        chartSegments.push(`
          <div style="text-align: center; border: 1px solid #30363d; border-radius: 8px; padding: 8px; background-color: #0d1117;">
            <div style="font-size: 9px; font-weight: bold; color: #8b949e; margin-bottom: 4px; text-transform: uppercase;">15 Min Chart</div>
      ...
      if (t.dailyChart) {
        chartSegments.push(`
          <div style="text-align: center; border: 1px solid #30363d; border-radius: 8px; padding: 8px; background-color: #0d1117;">
            <div style="font-size: 9px; font-weight: bold; color: #8b949e; margin-bottom: 4px; text-transform: uppercase;">Daily Chart</div>
      ...
      if (t.images && t.images.length > 0) {
        t.images.forEach((img, i) => {
          chartSegments.push(`
            <div style="text-align: center; border: 1px solid #30363d; border-radius: 8px; padding: 8px; background-color: #0d1117;">
              <div style="font-size: 9px; font-weight: bold; color: #8b949e; margin-bottom: 4px; text-transform: uppercase;">Screenshot #${i + 1}</div>
      ...
      const chartsGroupHtml = chartSegments.length > 0
        ...
        : `<div style="font-size: 10px; color: #8b949e; font-style: italic; margin-top: 5px;">No visual screenshot attachments uploaded for this execution.</div>`;

      return `
        <div class="trade-card" style="border: 1px solid #30363d; border-radius: 12px; padding: 18px; margin-bottom: 25px; background-color: #161b22; page-break-inside: avoid;">
          <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1.5px solid #30363d; padding-bottom: 8px; margin-bottom: 10px;">
            <div>
              <span style="font-weight: 800; font-size: 14px; color: #f0f6fc;">#${idx + 1} - ${t.pair}</span>
              <span style="background-color: ${t.direction === "LONG" ? "rgba(46, 160, 67, 0.15)" : "rgba(240, 48, 48, 0.15)"}; color: ${t.direction === "LONG" ? "#3fb950" : "#ff6b6b"}; border: 1px solid ${t.direction === "LONG" ? "rgba(46, 160, 67, 0.3)" : "rgba(240, 48, 48, 0.3)"}; border-radius: 4px; font-size: 10px; font-weight: 800; padding: 2px 7px; margin-left: 8px;">${t.direction}</span>
            </div>
            <div style="font-size: 14px; font-weight: 800; color: ${t.profit >= 0 ? "#00b074" : "#ff5252"};">
              ${t.profit >= 0 ? "+" : ""}$${t.profit.toLocaleString()}
            </div>
          </div>

          <!-- Parameters table & Details -->
          <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; background-color: #0d1117; padding: 10px; border-radius: 8px; font-family: monospace; font-size: 11px;">
            <div><span style="color: #8b949e; font-size: 9px; display: block;">DATE / TIME</span><strong>${t.date || "N/A"} ${t.time}</strong></div>
            <div><span style="color: #8b949e; font-size: 9px; display: block;">RISK DEPLOYED</span><strong>${t.riskPercent ? t.riskPercent + "%" : "N/A"}</strong></div>
            <div><span style="color: #8b949e; font-size: 9px; display: block;">LOT SIZE</span><strong>${t.lotSize ? t.lotSize : "N/A"} Lots</strong></div>
            <div><span style="color: #8b949e; font-size: 9px; display: block;">LEVERAGE / R:R</span><strong>${t.leverage ? t.leverage + "x" : "N/A"} / 1:${t.riskReward || "N/A"}</strong></div>
          </div>

          <!-- Entry Logic Badges -->
          <div style="margin-top: 10px;">
            <div style="font-size: 9px; text-transform: uppercase; font-weight: bold; color: #8b949e; margin-bottom: 4px;">Entry Rules Checklist:</div>
          ...
          <div style="margin-top: 10px; border-left: 3px solid #1f6feb; padding-left: 8px; font-size: 11px; line-height: 1.45; color: #c9d1d9;">
            <strong>Setup Narrative:</strong> ${t.entryReason || "No custom logic description provided."}
          </div>

          ${t.bigTimeFrameScenario ? `
          <div style="margin-top: 6px; border-left: 3px solid #d29922; padding-left: 8px; font-size: 11px; line-height: 1.45; color: #c9d1d9;">
            <strong>HTF Context:</strong> ${t.bigTimeFrameScenario}
          </div>` : ""}

          ${t.mistake && t.mistake.toLowerCase() !== "none" ? `
          <div style="margin-top: 8px; background-color: rgba(248, 81, 73, 0.1); border: 1px solid rgba(248, 81, 73, 0.2); border-radius: 6px; padding: 6px 10px; font-size: 11px; color: #ff7b72; display: flex; align-items: center; gap: 6px;">
            <strong>⚠️ Logged Pitfall / Mistake:</strong> ${t.mistake}
          </div>` : ""}

          <!-- Attached charts screenshots -->
          <div style="margin-top: 12px; border-top: 1px dashed #30363d; padding-top: 8px;">
            <strong style="font-size: 10px; color: #8b949e; text-transform: uppercase;">Visual Chart Attachments:</strong>
            ${chartsGroupHtml}
          </div>
        </div>
      `;
    ...
    const totalTradesHtml = `
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 25px;">
        <div style="border: 1px solid #30363d; background-color: #161b22; border-radius: 8px; padding: 12px; text-align: center;">
          <div style="font-size: 10px; color: #8b949e; text-transform: uppercase;">Executed Tickets</div>
          <div style="font-size: 20px; font-weight: 800; color: #f0f6fc;">${stratTrades.length}</div>
        </div>
        <div style="border: 1px solid #30363d; background-color: #161b22; border-radius: 8px; padding: 12px; text-align: center;">
          <div style="font-size: 10px; color: #8b949e; text-transform: uppercase;">Overall Win Rate</div>
          <div style="font-size: 20px; font-weight: 800; color: #58a6ff;">${winRate}%</div>
        </div>
        <div style="border: 1px solid #30363d; background-color: #161b22; border-radius: 8px; padding: 12px; text-align: center;">
          <div style="font-size: 10px; color: #8b949e; text-transform: uppercase;">Wins vs Losses</div>
          <div style="font-size: 20px; font-weight: 800; color: #c9d1d9;">${winTrades}W - ${lossTrades}L</div>
        </div>
        <div style="border: 1px solid #30363d; background-color: #161b22; border-radius: 8px; padding: 12px; text-align: center;">
          <div style="font-size: 10px; color: #8b949e; text-transform: uppercase;">Net Return</div>
          <div style="font-size: 20px; font-weight: 800; color: ${profitVal >= 0 ? "#00b074" : "#ff5252"};">
    ...
    reportWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div style="display: flex; justify-content: space-between; border-bottom: 2px solid #30363d; padding-bottom: 12px; margin-bottom: 20px;">
            <div>
              <strong style="font-size: 20px; letter-spacing: -0.5px; color: #58a6ff;">⚡ EDGEJOURNAL FRAMEWORK HUB</strong>
              <div style="font-size: 10px; color: #8b949e; text-transform: uppercase; margin-top: 3px; font-weight: bold;">Professional Strategic Edge Ledger</div>
            </div>
            <div style="text-align: right; font-size: 11px; font-family: monospace; color: #c9d1d9;">
          ...
          <div style="background-color: #161b22; border: 1.5px solid #30363d; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
            <h2 style="margin: 0 0 6px 0; font-size: 15px; color: #58a6ff;">${strat.name}</h2>
            <p style="margin: 0; font-size: 12px; color: #c9d1d9; line-height: 1.5;">
              <strong>Execution Blueprint Details:</strong> ${strat.description || "No specific blueprints logged."}
            </p>
          </div>

          <h3 style="font-size: 12px; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 15px; border-bottom: 1.5px solid #30363d; padding-bottom: 4px; color: #f0f6fc;">1. Aggregated Success Metrics</h3>
          ${totalTradesHtml}

          <h3 style="font-size: 12px; text-transform: uppercase; letter-spacing: 1px; margin-top: 30px; margin-bottom: 15px; border-bottom: 1.5px solid #30363d; padding-bottom: 4px; color: #f0f6fc;">2. Historical Trading Executions (${stratTrades.length} Trades)</h3>
          
          <div style="display: flex; flex-direction: column;">
            ${stratTrades.length > 0 ? tradeSheetsHtml : `<div style="text-align: center; color: #8b949e; padding: 40px; border: 1px dashed #30363d; border-radius: 8px; font-style: italic; font-size: 12px;">No trade executions registered under this strategy style. Log a trade in the journal ledger to populate this audit.</div>`}
          </div>

          <div style="margin-top: 50px; font-size: 10px; border-top: 1px dashed #30363d; padding-top: 15px; text-align: center; color: #8b949e;">
            ...
            {strategies.map((strat) => {
              ...
              return (
                <StaggerItem key={strat.id}>
                  <motion.div 
                    whileHover={springPresets.cardHover}
                    className="bg-[#161B22] border border-slate-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between transition duration-300 group relative overflow-hidden h-full"
                  >
                    ...
                    <div className="mt-5 pt-4 border-t border-slate-800/80 grid grid-cols-2 gap-2 text-center">
                      <div className="bg-[#0A0E14] p-2.5 rounded-xl border border-slate-800/80 group-hover:border-slate-700 transition duration-300">
                        ...
                        <span className="text-blue-400 text-sm font-black font-mono mt-0.5 block">
                      ...
                      <div className="bg-[#0A0E14] p-2.5 rounded-xl border border-slate-800/80 group-hover:border-slate-700 transition duration-300">
                        ...
                        <span className={`text-sm font-black font-mono mt-0.5 block ${pnl >= 0 ? "text-[#00b074]" : "text-rose-500"}`}>
                          <AnimatedCounter value={pnl} isCurrency={true} prefix={pnl >= 0 ? "+$" : "$"} />
                        </span>
                      </div>
                    </div>
