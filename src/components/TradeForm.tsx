          ...
          if (file) {
            ...
            reader.onloadend = () => {
              ...
              if (activePasteSlot === "daily") {
                ...
                showToast("📅 Daily Chart pasted successfully!");
              } else if (activePasteSlot === "4h") {
                setFourHourChart(base64Data);
                showToast("⏱️ 4 Hour Chart pasted successfully!");
              } else if (activePasteSlot === "1h") {
                setOneHourChart(base64Data);
                showToast("⚡ 1 Hour Chart pasted successfully!");
              } else if (activePasteSlot === "15m") {
                setFifteenMinChart(base64Data);
                showToast("🎯 15 Min Chart pasted successfully!");
              } else {
                setImages((prev) => [...prev, base64Data]);
                showToast("📸 Supporting screenshot attached!");
              }
            };
    ...
    if (screenshotBase64) {
      if (activePasteSlot !== "generic") {
        if (activePasteSlot === "daily") {
          ...
          showToast("Gemini parsed Daily Chart!");
        } else if (activePasteSlot === "4h") {
          setFourHourChart(screenshotBase64);
          showToast("Gemini parsed 4H Chart!");
        } else if (activePasteSlot === "1h") {
          setOneHourChart(screenshotBase64);
          showToast("Gemini parsed 1H Chart!");
        } else if (activePasteSlot === "15m") {
          setFifteenMinChart(screenshotBase64);
          showToast("Gemini parsed 15M Chart!");
        }
      } else {
        // Smart Cascading Attachment routes screenshot to the first unpopulated block!
        if (!dailyChart) {
          setDailyChart(screenshotBase64);
          showToast("Gemini screenshot auto-saved as Daily Chart!");
        } else if (!fourHourChart) {
          setFourHourChart(screenshotBase64);
          showToast("Gemini screenshot auto-saved as 4 Hour Chart!");
        } else if (!oneHourChart) {
          setOneHourChart(screenshotBase64);
          showToast("Gemini screenshot auto-saved as 1 Hour Chart!");
        } else if (!fifteenMinChart) {
          setFifteenMinChart(screenshotBase64);
          showToast("Gemini screenshot auto-saved as 15 Min Chart!");
        } else {
          setImages((prev) => [...prev, screenshotBase64]);
          showToast("Gemini screenshot appended as support slide!");
        }
      }
    } else {
  ...
  const handleImageFileLoad = (e: React.ChangeEvent<HTMLInputElement>, category: "daily" | "4h" | "1h" | "15m" | "generic") => {
    ...
    if (files && files[0]) {
      ...
      reader.onloadend = () => {
        ...
        if (category === "daily") {
          setDailyChart(base64);
          showToast("📅 Daily Chart uploaded!");
        } else if (category === "4h") {
          setFourHourChart(base64);
          showToast("⏱️ 4 Hour Chart uploaded!");
        } else if (category === "1h") {
          setOneHourChart(base64);
          showToast("⚡ 1 Hour Chart uploaded!");
        } else if (category === "15m") {
          setFifteenMinChart(base64);
          showToast("🎯 15 Min Chart uploaded!");
        } else {
          setImages((prev) => [...prev, base64]);
          showToast("📷 Supporting slide uploaded!");
        }
      };
